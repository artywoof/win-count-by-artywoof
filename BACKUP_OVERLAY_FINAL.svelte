<script lang="ts">
  ...
  // Simple overlay state - just what we need to display
  let currentWin = 0;
  let currentGoal = 10;
  let showCrown = true;
  let showGoal = true;

  // Initialize overlay - listen for updates from main app
  onMount(() => {
    // Listen for state updates via BroadcastChannel
    if (typeof BroadcastChannel !== 'undefined') {
      const overlayChannel = new BroadcastChannel('win-count-overlay');
      overlayChannel.onmessage = (event) => {
        const data = event.data;
        
        // Update overlay state from main app
        if (data.win !== undefined) currentWin = data.win;
        if (data.goal !== undefined) currentGoal = data.goal;
